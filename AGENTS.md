...
- Assume the reader already lifts, sleeps, does not smoke, and is not asking whether stress is bad.
- No hand-holding. No “talk to your doctor” on every claim. No “early adopter action ladder.”
- No equity sermons, access lectures, stigma language, identity politics, or “societal implications” filler. Biology, evidence, practice, uncertainty. That is the whole beat.
- Optimism is allowed. Cheerleading and fearmongering are not. A result can be exciting and still thin.

## The job

1. Find what is known, what is published, what people are doing, and what is still a guess.
2. Put it in writing with a source on every claim.
...
Every statement that asserts something starts with one of these. Titles, section headers, and bare citations do not. If a sentence mixes evidence grades, split it.
...
| 📜 | Preprint. Posted, not peer-reviewed. | bioRxiv, medRxiv, arXiv, and the like. A real manuscript, no journal stamp yet. |
| 🥼 | Papers in a live fight. | Research literature disagrees — published, preprint, or both. Cite both sides. Do not pick a winner to look tidy. |
...
- 💯 is expensive. A famous paper is not 💯. Consensus across the field, with no live dispute, is 💯.
- One paper, even in *Nature*, is 📚.
- A preprint is 📜, not 📚, not 🤔. Do not promote it because the authors are famous. Do not dump it into 🐉 just because it has not been reviewed.
...
```
💯 DNA lesions that persist through replication can become mutations.
📚 Annual somatic mutation rate in intestinal crypts scales inversely with lifespan across 16 mammals (Cagan et al., Nature, 2022).
📜 F. vesiculosus fucoidan activated SIRT6 and extended male mouse lifespan in a 2025 preprint (Gorbunova et al., bioRxiv).
🥼 Ordinary point-mutation burden as the master clock of human aging is not settled; persistent lesions, transcription blocks, senescence, and clonal selection are live alternatives in the literature.
...
Hallucinations are not tolerated. If you cannot point to a source, you do not get to say it.

- Every marked statement needs a citation or an explicit “no source; inference from X” tagged 🐉.
- Prefer primary papers over reviews, reviews over news, news over memory. Fetch the paper. Do not cite from vibes.
- Record author, year, venue, and a URL, DOI, or PMID. Quantify: effect size, N, population, endpoint, duration.
- Forum and clinic sources are valid for *practice*, not for *efficacy*. Keep those jobs separate.
- Commercial conflicts get a clause, not a sermon (“X sells the capsule they are citing”).
- If a source cannot be found again, drop the claim. Invented citations are a firing offense.

The scientific community’s “wait for the perfect RCT in 2048” stance is not a veto. It is one input. 📜, 🤔, and 🐉 exist so we can write the rest without pretending it is 📚.
...
- Fake certainty. Fake humility that buries the finding.
...
compounds/<slug>/sources/<emoji>/                    same mark dirs as hallmarks
scripts/build-index.py                               regenerate catalog / section-map blocks
...
| Dir | Hallmark |
|---|---|
| `hallmarks/01-genomic-instability/` | Genomic instability |
| `hallmarks/02-telomere-attrition/` | Telomere attrition |
| `hallmarks/03-epigenetic-alterations/` | Epigenetic alterations |
| `hallmarks/04-loss-of-proteostasis/` | Loss of proteostasis |
| `hallmarks/05-disabled-macroautophagy/` | Disabled macroautophagy |
| `hallmarks/06-deregulated-nutrient-sensing/` | Deregulated nutrient sensing |
| `hallmarks/07-mitochondrial-dysfunction/` | Mitochondrial dysfunction |
| `hallmarks/08-cellular-senescence/` | Cellular senescence |
| `hallmarks/09-stem-cell-exhaustion/` | Stem-cell exhaustion |
| `hallmarks/10-altered-intercellular-communication/` | Altered intercellular communication |
| `hallmarks/11-chronic-inflammation/` | Chronic inflammation |
| `hallmarks/12-dysbiosis/` | Dysbiosis |
...
- Lead with the claim, not the throat-clearing.
- Separate mechanism, animal data, human data, and what clinics/self-experimenters are doing.
- A biomarker move is not a healthspan result. Say so without a paragraph of scolding.
- Negative, null, and harm evidence stay in the same section as the positive claims.
- Speculative sections are required, not an apology appendix. Mark them 🐉 or 🤔 and move on.
- One not-medical-advice paragraph per document. Never again in that file.
