...
| `.cursor/`, `.claude/` | Hook/agent/skill/rule conventions; `36-no-regex-semantic-intent`; `50-self-improve`; create-hook / create-subagent layout if those files changed. Hooks must not classify user intent by keywords. `paper-hunter` (`.cursor/agents/paper-hunter.md`) is fetch/patch on the legal OA ladder; pirate-mirror fetch is a fail. |
